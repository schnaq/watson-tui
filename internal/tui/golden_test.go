...
//
// Everything the frame is built from is fixed: a frozen clock, three frames
// spelled out with explicit dates, and the "alle Frames" period, so no boundary
// of the real calendar can move a row. The frames are built in time.Local
// because the views format in local time — building them in UTC would make the
// rendered clock times depend on the TZ of whoever runs the test. The dates sit
// in July for the same kind of reason: no zone changes its offset then, so
// "09:00 local" is 09:00 everywhere.
