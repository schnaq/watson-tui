...
func TestOverviewColumns(t *testing.T) {
	...
	titles := []string{"this week", "last week", "this month", "last month", "all"}
	for i, want := range titles {
		if cols[i].title != want {
			t.Errorf("col %d = %q, want %q", i, cols[i].title, want)
		}
	}
	// last week ends where this week begins
	...
	if int(juneFrom.Month()) != 6 || juneFrom.Day() != 1 {
		t.Errorf("last month from=%v, want 2026-06-01", juneFrom)
	}
	// all is unbounded
...
func TestBuildOverview(t *testing.T) {
	...
	// sorted by the all column desc: alpha (5h), gamma (3h), beta (2h)
